...
if (typeof CanvasRenderingContext2D !== 'undefined' && !CanvasRenderingContext2D.prototype.roundRect) {
  CanvasRenderingContext2D.prototype.roundRect = function (x, y, w, h, r) {
    if (Array.isArray(r)) r = r[0]; // Simplification for our usage
    this.moveTo(x + r, y);
...
const ASSET_MAP = {
  ...
  city: 'city.png', lotusTower: 'lotus%20tw.png', temple: 'temple.png',
  ...
  fuelShed1: 'Fuel%20man/fuel%20shed%201.png', fuelShed2: 'Fuel%20man/fuel%20shed%20%202.png',
  dalanda: 'femus/dalanda%20maligawa.png', sigiriya: 'femus/sigiriya.png',
  ...
  skyCloud1: 'sky cloud/sky cloud 1.png',
  skyCloud2: 'sky cloud/sky cloud 2.png',
  skyCloud3: 'sky cloud/sky cloud  3.png',

  // City Layers
  city1: 'city%201/city%201.png',
  cityL1: 'city%201/city-2_0000_Layer-1.png', cityL2: 'city%201/city-2_0001_Layer-2.png',
  cityL3: 'city%201/city-2_0002_Layer-3.png', cityL4: 'city%201/city-2_0003_Layer-4.png',
  cityL5: 'city%201/city-2_0004_Layer-5.png', cityL6: 'city%201/city-2_0005_Layer-6.png',
  city3L1: 'city%201/city-3-_0000_Layer-1.png', city3L2: 'city%201/city-3-_0001_Layer-2.png',
  city3L3: 'city%201/city-3-_0002_Layer-3.png',
  city4L1: 'city%201/city-4_0000_Layer-1.png', city4L2: 'city%201/city-4_0001_Layer-2.png',
  city4L3: 'city%201/city-4_0002_Layer-3.png',

  // Billboards
  bb1: 'bill%20board/b%20board%201.png', bb2: 'bill%20board/b%20board%20%202.png',
  bb3: 'bill%20board/b%20board%20%203.png', bb4: 'bill%20board/b%20board%204.png',
  bb5: 'bill%20board/b%20board%205.png', bb6: 'bill%20board/b%20board%206.png',

  // Village
  vh1_1: 'v%20house/village-house_0000_Layer-1.png', vh1_2: 'v%20house/village-house_0001_Layer-2.png',
  vh1_0: 'v%20house/village-house_0002_Layer-0.png',
  vh2_1: 'v%20house/village-house-2_0000_Layer-1.png', vh2_2: 'v%20house/village-house-2_0001_Layer-2.png',
  vh3_1: 'v%20house/village-house-3_0000_Layer-1.png', vh3_2: 'v%20house/village-house-3_0001_Layer-2.png',

  // Shops
  shop1: 'shop/shop%201.png', shop2: 'shop/shop%202.png', shop3: 'shop/shop%203.png',

  // Nature Extended
  natureL1: 'nature%20env/nature-1_0000_Layer-1.png', natureL2: 'nature%20env/nature-1_0001_Layer-2.png',
  natureL3: 'nature%20env/nature-1_0002_Layer-3.png', natureL4: 'nature%20env/nature-1_0003_Layer-4.png',
  natureL5: 'nature%20env/nature-1_0004_Layer-5.png',
  tree1_1: 'trees/trees-1_0000_Layer-1.png', tree1_0: 'trees/trees-1_0001_Layer-0.png',
  tree2_1: 'trees/trees-2_0000_Layer-1.png', tree2_0: 'trees/trees-2_0001_Layer-0.png',
  tree3_1: 'trees/trees-3_0000_Layer-1.png', tree3_0: 'trees/trees-3_0001_Layer-0.png',
  tree4_1: 'trees/trees-4_0000_Layer-1.png', tree4_0: 'trees/trees-4_0001_Layer-0.png',
  tree5_1: 'trees/trees-5_0000_Layer-1.png', tree5_0: 'trees/trees-5_0001_Layer-0.png',
  tree6_1: 'trees/trees-6_0000_Layer-1.png', tree6_0: 'trees/trees-6_0001_Layer-0.png',
...
class Camera {
  ...
  follow(tx, ty, canvasW, canvasH, speed) {
    // Cinematic gameplay zoom: Higher base zoom for close-up, zooms out as speed increases
    const targetZoom = Math.max(0.85, 1.25 - (speed / 30));
...
class AudioManager {
  ...
  update(bike, input, scene) {
    ...
    const gas = bike.autoDrive ? 0.45 : (input.fwd ? 1 : (input.bwd ? -1 : 0));
    const brake = bike.autoDrive ? false : input.bwd;
    const accelerating = gas > 0 && isActive;
    const braking = brake && this._smoothSpeed > 1 && isActive;
...
class Terrain {
  ...
  draw(ctx, am) {
    ...
    ctx.lineTo(lastPt.x, lastPt.y + 1200); ctx.lineTo(this.pts[0].x, this.pts[0].y + 1200); ctx.closePath();

    // ── Smoother Soil Blending ──────────────────────────────
    // Gradient starts from a darker brown (matching road bottom) to a very deep earth tone
    const soilGrd = ctx.createLinearGradient(0, 500, 0, 1100);
    soilGrd.addColorStop(0, '#3a1c08'); // Upper soil (matches road bottom)
    soilGrd.addColorStop(0.2, '#2a1406'); // Deep soil
    soilGrd.addColorStop(1, '#1a0d04');   // Infinite depth
    ctx.fillStyle = soilGrd; ctx.fill();
...
class Bike {
  ...
  update(input) {
    ...
    // ── Engine ──────────────────────────────────────────────
    const isFwd = this.autoDrive ? true : input.fwd;
    const isBwd = this.autoDrive ? false : input.bwd;
...
const PROPS_DATA = [
  ...
  {
    "asset": "v house/village-house_0001_Layer-2.png",
    "label": "V-House 1-2",
    "layer": "bg",
    "x": 7392,
    "y": 338,
  ...
  {
    "asset": "v house/village-house_0000_Layer-1.png",
    "label": "V-House 1-1",
    "layer": "bg",
    "x": 7182,
    "y": 382,
  ...
  {
    "asset": "trees/trees-4_0001_Layer-0.png",
    "label": "Tree 4-0",
    "layer": "bg",
    "x": 4659,
    "y": 189,
  ...
  {
    "asset": "city 1/city-2_0002_Layer-3.png",
    "label": "City 2 L3",
    "layer": "bg",
    "x": -659,
    "y": -48,
  ...
  {
    "asset": "bill board/b board  3.png",
    "label": "B-Board 3",
    "layer": "bg",
    "x": 570,
    "y": 358,
  ...
  {
    "asset": "bill board/b board 6.png",
    "label": "B-Board 6",
    "layer": "bg",
    "x": 1692,
    "y": 56,
  ...
  {
    "asset": "trees/trees-6_0001_Layer-0.png",
    "label": "Tree 6-0",
    "layer": "bg",
    "x": 2467,
    "y": 232,
  ...
  {
    "asset": "nature env/nature-1_0004_Layer-5.png",
    "label": "Nature L5",
    "layer": "bg",
    "x": 2945,
    "y": 454,
  ...
  {
    "asset": "v house/village-house-3_0000_Layer-1.png",
    "label": "V-House 3-1",
    "layer": "bg",
    "x": 4342,
    "y": 295,
  ...
  {
    "asset": "trees/trees-1_0000_Layer-1.png",
    "label": "Tree 1-1",
    "layer": "bg",
    "x": 3360,
    "y": 97,
  ...
  {
    "asset": "trees/trees-4_0000_Layer-1.png",
    "label": "Tree 4-1",
    "layer": "bg",
    "x": 4618,
    "y": 179,
  ...
  {
    "asset": "trees/trees-5_0001_Layer-0.png",
    "label": "Tree 5-0",
    "layer": "bg",
    "x": 5138,
    "y": 188,
  ...
  {
    "asset": "v house/village-house-3_0001_Layer-2.png",
    "label": "V-House 3-2",
    "layer": "bg",
    "x": 4910,
    "y": 338,
  ...
  {
    "asset": "bill board/b board  2.png",
    "label": "B-Board 2",
    "layer": "bg",
    "x": 5265,
    "y": 396,
...
class Environment {
  constructor(terrain) {
    ...
    if (PROPS_DATA && PROPS_DATA.length > 0) {
      PROPS_DATA.forEach(p => {
        let key = Object.keys(ASSET_MAP).find(k => {
          const fullPath = decodeURIComponent(ASSET_MAP[k]);
          const pAsset = decodeURIComponent(p.asset);
          return fullPath === pAsset ||
            fullPath.split('/').pop() === pAsset ||
            fullPath === pAsset.split('/').pop();
        });
        if (key) {
          this.props.push({
            key, x: p.x, y: p.y,
  ...
  drawBG(ctx, am, cam, canvasW, canvasH, gameScale) {

    const vW = canvasW / gameScale;
    const vH = canvasH / gameScale;

    // 1. High-Fidelity Day Sky Gradient
    const sky = ctx.createLinearGradient(0, 0, 0, vH);
    sky.addColorStop(0, '#1a5fff');     // Vibrant Deep Azure
    sky.addColorStop(0.4, '#7bb8ff');   // Light Sky Blue
    sky.addColorStop(0.7, '#e0f1ff');   // Pale daylight
    sky.addColorStop(1, '#ffffff');     // Bright Horizon
    ctx.fillStyle = sky; ctx.fillRect(0, 0, vW, vH);

    // 2. Glowing Sun Rendering
    const sunX = Math.min(vW * 0.8, 550);
    const sunY = vH * 0.22;
    ...
    sunCore.addColorStop(0.2, '#fff4cc');
    sunCore.addColorStop(0.5, 'rgba(255, 230, 150, 0.8)');
    sunCore.addColorStop(1, 'rgba(255, 255, 255, 0)');
    ctx.fillStyle = sunCore;
    ctx.beginPath(); ctx.arc(sunX, sunY, 60, 0, Math.PI * 2); ctx.fill();

    // 3. Sky Cloud Assets (Replacing procedural ellipses)
    ctx.save();
    const ct = Date.now() * 0.00002;

    // Distant Clouds (Small/Slow)
    ctx.globalAlpha = 0.4;
    const cx1 = -(cam.x * 0.02 + ct * 1200) % 2500;
    const cloud1 = am.get('skyCloud1');
    if (cloud1) {
      for (let i = 0; i < 6; i++) {
        const x = ((cx1 + i * 600) % 2500) - 300;
        ctx.drawImage(cloud1, x, vH * 0.1 + Math.sin(i) * 30, cloud1.width * 0.5, cloud1.height * 0.5);
      }
    }

    // Mid Clouds
    ctx.globalAlpha = 0.6;
    const cx2 = -(cam.x * 0.06 + ct * 2000) % 2500;
    const cloud2 = am.get('skyCloud2');
    if (cloud2) {
      for (let i = 0; i < 5; i++) {
        const x = ((cx2 + i * 800) % 2500) - 400;
        ctx.drawImage(cloud2, x, vH * 0.3 + Math.cos(i) * 40, cloud2.width * 0.7, cloud2.height * 0.7);
      }
    }

    // Larger Nearer Clouds
    ctx.globalAlpha = 0.8;
    const cx3 = -(cam.x * 0.1 + ct * 3000) % 2500;
    const cloud3 = am.get('skyCloud3');
    if (cloud3) {
      for (let i = 0; i < 4; i++) {
        const x = ((cx3 + i * 1000) % 2500) - 500;
        ctx.drawImage(cloud3, x, vH * 0.15 + Math.sin(i * 1.5) * 50, cloud3.width * 1.0, cloud3.height * 1.0);
      }
    }
    ctx.restore();

    // Ambient floating particles (Heat shimmer / dust)
    this.drawAmbient(ctx, cam, vW, vH);

    // 4. Distant Mountains (Parallax 0.02)
    const mx = -cam.x * 0.02;
    ctx.fillStyle = 'rgba(180, 210, 255, 0.7)'; // Hazy Blue Mountains
    this._mtns(ctx, mx, vH * 0.55, vH * 0.3, 6, 1234);

    // 5. Far Hills (Parallax 0.15)
    const hx = -cam.x * 0.15;
    ctx.fillStyle = 'rgba(140, 190, 230, 0.85)'; // Slightly clearer hills
    ctx.beginPath(); ctx.moveTo(0, vH * 0.73);
    for (let x = 0; x <= vW; x += 10) {
      ctx.lineTo(x, vH * 0.73 - Math.sin((x + hx) * 0.04) * 45 - 20);
    }
...
class IntroScene {
  ...
  drawBubble(ctx, x, y, text, type) {
    ...
    const padding = 25;
    const lines = this.getLines(ctx, text, 400); // Split by max width of 400
    const lineH = 34;

    // Calculate actual maxWidth among lines
    let maxLW = 0;
    lines.forEach(l => { const mw = ctx.measureText(l).width; if (mw > maxLW) maxLW = mw; });

    const w = Math.max(120, maxLW + padding * 2.5);
    const h = lines.length * lineH + padding * 2;
    const bx = x - w / 2, by = y - h - 30;

    // Shadow & Styling
    ctx.shadowBlur = 20; ctx.shadowColor = 'rgba(0,0,0,0.25)';
    ctx.fillStyle = '#ffffff';
    ctx.strokeStyle = '#000000';
    ctx.lineWidth = 2.5;

    // Both types now use the rounded rect style box
    this.roundRect(ctx, bx, by, w, h, 24);
    ctx.fill();
    ctx.stroke();

    ctx.shadowBlur = 0; // Clear shadow for tail
    ...
    if (type === 'thinking') {
      ...
      ctx.beginPath();
      ctx.arc(x - 20, by + h + 15, 12, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
      ctx.beginPath();
      ctx.arc(x - 45, by + h + 45, 8, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
    } else {
      // Speech tail (Triangle)
      ctx.beginPath();
      ctx.moveTo(x + 20, by + h);
      ctx.lineTo(x + 50, by + h + 35);
      ctx.lineTo(x + 80, by + h);
      ctx.fill();
      ctx.stroke();

      // Patch the stroke line where tail meets box
      ctx.beginPath();
      ctx.lineWidth = 4;
      ctx.strokeStyle = '#fff';
      ctx.moveTo(x + 22, by + h);
      ctx.lineTo(x + 78, by + h);
      ctx.stroke();
    }
    ...
    lines.forEach((l, i) => {
      ctx.fillText(l, x, by + padding + 22 + i * lineH);
    });
  ...
  draw(ctx, vW, vH, am, cam, bike, env) {
    this.timer++;
    const fuelMan = env.props.find(p => p.key === 'fuelMan');

    // Cinematic Zoom: High during dialogue, medium during ride, normal for game
    let targetZoom = 1.0;
    if (this.state === 'DIALOGUE') targetZoom = 1.7;
    else if (this.state === 'AUTO_DRIVE') targetZoom = 1.6;
    cam.zoom += (targetZoom - cam.zoom) * 0.035;

    // Calculate fuel progress to be 100% at end of last dialogue
    const totalD = this.dialogue.length;
    const currentProg = (this.step * 180 + this.timer) / (totalD * 180);
    this.fuelProgress = Math.min(1, currentProg);
    bike.fuel = this.fuelProgress;

    // ── GAUGE SMOOTH FADE LOGIC ────────────────────────────────
    if (this.state === 'AUTO_DRIVE') {
      this.gaugeAlpha = Math.max(0, this.gaugeAlpha - 0.025);
    } else {
      this.gaugeAlpha = 1.0;
    }

    if (this.gaugeAlpha > 0) {
      ctx.save();
      ctx.globalAlpha *= this.gaugeAlpha;

      // ── VERTICAL LIQUID FUEL GAUGE (Right of Fuel Man) ──────────
      const gW = 35, gH = 320;
      const gx = (fuelMan ? fuelMan.x + 360 : 750) - cam.x;
      const roadY = (this.game.terrain ? this.game.terrain.surfaceY(gx + cam.x) : 600);
      const gy = roadY - cam.y - gH;

      ctx.save();

      // Glass Container
      ctx.shadowBlur = 15; ctx.shadowColor = 'rgba(255, 255, 255, 0.2)';
      ctx.fillStyle = 'rgba(255, 255, 255, 0.15)';
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.5)';
      ctx.lineWidth = 2;
      this.roundRect(ctx, gx, gy, gW, gH, 10);
      ctx.fill(); ctx.stroke();
      ctx.shadowBlur = 0;

      // Liquid Fill (Bottom Up)
      const fillH = gH * this.fuelProgress;
      const fillY = gy + gH - fillH;

      if (fillH > 5) {
        const fillGrd = ctx.createLinearGradient(gx, gy + gH, gx, gy);
        fillGrd.addColorStop(0, '#ff9900'); fillGrd.addColorStop(1, '#ffcc00');
        ctx.fillStyle = fillGrd;

        // Liquid Surface Wave Animation
        const waveSpeed = Date.now() * 0.006;
        const waveAmp = 4;
        const waveFreq = 0.15;
        ...
        for (let i = 0; i <= gW; i += 2) {
          const wave = Math.sin(i * waveFreq + waveSpeed) * waveAmp;
          ctx.lineTo(gx + i, fillY + wave);
        }
      ...
      ctx.lineWidth = 1;
      for (let i = 1; i < 10; i++) {
        const my = gy + gH - (gH * (i / 10));
        ctx.beginPath(); ctx.moveTo(gx + gW - 10, my); ctx.lineTo(gx + gW, my); ctx.stroke();
      }
      ...
      ctx.fillText("FUEL TANK", 0, 0);
      ctx.restore();

      ctx.font = '700 15px Outfit';
      ctx.fillText(`${(this.fuelProgress * 100).toFixed(0)}%`, gx + gW / 2, gy + gH + 25);

      ctx.restore();
      ctx.restore(); // Final balance for gaugeAlpha save
    } // Close gaugeAlpha > 0
    ...
    if (this.state === 'DIALOGUE') {
      ...
      if (d.speaker === 'fuelman') {
        targetX = (fuelMan ? fuelMan.x + 150 : 250) - cam.x;
        targetY = (fuelMan ? fuelMan.y - 80 : 350) - cam.y;
      } else {
        targetX = (bike.x + 40) - cam.x;
        targetY = (bike.y - 280) - cam.y;
      }

      this.drawBubble(ctx, targetX, targetY, d.text, d.speaker === 'rider' ? 'thinking' : 'speech');

      if (this.timer > 180) {
        this.step++;
        this.timer = 0;
        if (this.step >= this.dialogue.length) {
          this.state = 'AUTO_DRIVE';
          bike.autoDrive = true;
        }
      }
    } else if (this.state === 'AUTO_DRIVE') {
      if (bike.x > 700) {
        bike.autoDrive = false;
        bike.speed = 0;
        bike.x = 700.1; // Snap precisely to start line
        this.state = 'READY';
...
class Game {
  ...
  _resize() {
    ...
    this.baseScale = this.scale; // Store original fitting scale
  ...
  switchScene(name) {
    if (name === 'game' || name === 'intro') {
      ...
      if (name === 'intro') {
        ...
        this.bike.x = 700; // Normal start
      }
    }
    this.scene = 'game'; // Intro is now part of Game scene
  }
  _initGame() {
    ...
    const startX = 100; // At gas station
    const sy = this.terrain.surfaceY(startX);
    this.bike = new Bike(startX, sy - 50, this.terrain);
    this.env = new Environment(this.terrain);
    this.cam = new Camera(); this.cam.x = -100; this.cam.y = sy - C.H * .5;
    this.introCamWeight = 0; // For smooth transitions
  ...
  _loop() {
    ...
    // Safety Checks
    if (isNaN(this.scale)) this.scale = 0.5;
    if (this.cam && isNaN(this.cam.zoom)) this.cam.zoom = 1.0;
    if (this.cam && isNaN(this.cam.x)) this.cam.x = 0;
    if (this.cam && isNaN(this.cam.y)) this.cam.y = 0;

    const dpr = window.devicePixelRatio || 1;
    const currentScale = this.scale * ((this.scene === 'game' && this.cam) ? this.cam.zoom : 1.0);
    const vW = window.innerWidth / currentScale;
    const vH = window.innerHeight / currentScale;

    ctx.setTransform(1, 0, 0, 1, 0, 0); // Reset transform every frame
    ctx.save();
    try {
      ...
      // Apply Camera Shake
      if (this.scene === 'game' && this.cam && this.cam.shake > 0.1) {
      ...
      if (this.scene === 'loading') {
        ...
        this.intro.draw(ctx, vW, vH, this.am);
      } else if (this.scene === 'game') {
        this._updateGame();
        this._drawGame(ctx, vW, vH);
        if (this.introActive && this.intro) {
          this.intro.draw(ctx, vW, vH, this.am, this.cam, this.bike, this.env);
        }
      ...
      if (this.audio && this.scene !== 'loading') {
        if (this.intro && (this.intro.state !== 'READY' && this.intro.state !== 'AUTO_DRIVE')) {
        } else {
          try { this.audio.update(this.bike, this.input, this.scene); } catch (e) { }
        }
      }
    } catch (err) {
      // Emergency Error Display for debug
      ctx.setTransform(1, 0, 0, 1, 0, 0);
      ctx.fillStyle = 'white'; ctx.fillRect(0, 0, 600, 100);
      ctx.fillStyle = 'red'; ctx.font = '16px monospace';
      ctx.fillText("DRAW ERROR: " + err.message, 10, 30);
      console.error(err);
    } finally {
      ctx.restore();
    }
  ...
  _updateGame() {
    ...
    let tx = this.bike.pos.x, ty = this.bike.pos.y;

    // Smooth Transition between Intro Focus and Bike Focus
    let targetWeight = 0;
    if (this.introActive && this.intro) {
      if (this.intro.state === 'DIALOGUE') targetWeight = 1.0;
      else if (this.intro.state === 'AUTO_DRIVE') targetWeight = 0.3;
      else targetWeight = 0;
    }

    if (this.introCamWeight === undefined) this.introCamWeight = 0;
    this.introCamWeight += (targetWeight - this.introCamWeight) * 0.05;

    if (this.introCamWeight > 0.01) {
      const fuelMan = this.env.props.find(p => p.key === 'fuelMan');
      if (fuelMan) {
        const introTargetX = (this.bike.x + fuelMan.x + 150) / 2;
        const introTargetY = (this.bike.y + fuelMan.y - 120) / 2;
        tx = tx * (1 - this.introCamWeight) + introTargetX * this.introCamWeight;
        ty = ty * (1 - this.introCamWeight) + introTargetY * this.introCamWeight;
      }
    }

    this.cam.follow(tx, ty, vW, vH, this.bike.speed);
    // Dust & Exhaust
  ...
  _drawGame(ctx, vW, vH) {
    ...
    this.env.drawBG(ctx, this.am, this.cam, vW, vH, 1);
